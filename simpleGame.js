...
function Animation(spriteSheet, imgWidth, imgHeight, cellWidth, cellHeight) {
  ...
  this.drawFrame = function (ctx) {
    ...
    // Scale the drawing
    const scaleFactor = 3; // Increase size by 3x (change this to 4 for 4x size)
    ctx.save(); // Save the current state of the canvas
    ctx.translate(this.cellWidth / 2, this.cellHeight / 2); // Translate to the center of the sprite
    ctx.scale(scaleFactor, scaleFactor); // Scale by factor (3x or 4x)
    ctx.translate(-this.cellWidth / 2, -this.cellHeight / 2); // Translate back to top-left corner
